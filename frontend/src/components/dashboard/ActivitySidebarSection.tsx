...
const ActivitySidebarSection = ({
  ...
  return (
    <div className="border-t border-[#2A2D3C] bg-[#0F1117] px-4 py-5 flex flex-col flex-1">
      <div className="flex items-center justify-between mb-5">
        <h3 className="flex items-center gap-2 text-base font-bold text-slate-100">
          <Bell className="h-5 w-5 text-primary" /> Activity
        </h3>
        <button
          onClick={onClose}
      ...
      <div className="space-y-5 overflow-y-auto flex-1 pr-2">
              ...
              {notifications.slice(0, 8).map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-lg border px-3 py-2.5 text-left transition text-xs ${
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {unread && <span className="h-2 w-2 rounded-full bg-[#BED234] flex-shrink-0" />}
                    </div>
                    <p className="text-[11px] text-slate-400 line-clamp-2 mt-1">{item.body}</p>
                  </button>
                );
              })}
            </div>
        ...
        {followIds.length > 0 && (
          <section className="space-y-3">
            ...
            <p className="text-sm text-slate-400">Following {followIds.length} entities</p>
            <div className="flex flex-wrap gap-2">
              {followIds.slice(0, 6).map((id) => (
                <Badge key={id} variant="secondary" className="border border-[#2A2D3C] bg-[#1A1D2B] text-[10px] text-slate-300 px-2.5 py-1.5">
                  {id.slice(0, 14)}...
                </Badge>
              ))}
            </div>
        ...
        {savedPosts.length > 0 && (
          <section className="space-y-3">
            <h4 className="flex items-center gap-2 text-sm font-semibold text-slate-200">
              <Clock3 className="h-4 w-4 text-primary" /> Saved
            </h4>
            <div className="space-y-2.5">
              {savedPosts.slice(0, 6).map((post) => (
                <button
                  ...
                  className="w-full rounded-lg border border-[#2A2D3C] bg-[#1A1D2B]/70 px-3 py-2.5 transition hover:border-[#BED234]/40 hover:bg-[#1A1D2B] text-left"
                >
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {post.author && (
                        <p className="text-[11px] text-slate-400 mt-1">by {post.author}</p>
                      )}
                    </div>
                  </div>
                </button>
